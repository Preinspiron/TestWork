...
import {
  ...
  withEmotionCache
} from "./chunk-ZE7HGTFT.js";
import {
  _extends,
  _objectWithoutPropertiesLoose
} from "./chunk-PVA4JIPA.js";
import {
  require_react_dom
...
var ThemeContext2 = React73.createContext(null);
if (true) {
...
function useCurrentColorScheme(options) {
  ...
  const setMode = React78.useCallback((mode) => {
  ...
  const setColorScheme = React78.useCallback((value) => {
    if (!value) {
      setState((currentState) => {
        try {
  ...
  const handleMediaQuery = React78.useCallback((e) => {
  ...
  const mediaListener = React78.useRef(handleMediaQuery);
...
function createCssVarsProvider(options) {
  ...
  const ColorSchemeContext = React79.createContext(void 0);
  const useColorScheme = () => {
    const value = React79.useContext(ColorSchemeContext);
    if (!value) {
  ...
  function CssVarsProvider({
    ...
    const hasMounted = React79.useRef(false);
    const upperTheme = useTheme3();
    const ctx = React79.useContext(ColorSchemeContext);
    ...
    const element = (0, import_jsx_runtime50.jsxs)(React79.Fragment, {
      children: [shouldGenerateStyleSheet && (0, import_jsx_runtime50.jsxs)(React79.Fragment, {
...
function joinChildren(children2, separator) {
  const childrenArray = React82.Children.toArray(children2).filter(Boolean);
...
function Ripple(props) {
  ...
  const [leaving, setLeaving] = React89.useState(false);
...
var TouchRipple = React90.forwardRef(function TouchRipple2(inProps, ref) {
  ...
  const [ripples, setRipples] = React90.useState([]);
  const nextKey = React90.useRef(0);
  const rippleCallback = React90.useRef(null);
  React90.useEffect(() => {
  ...
  const ignoringMouseDown = React90.useRef(false);
  const startTimer = React90.useRef(null);
  const startTimerCommit = React90.useRef(null);
  const container = React90.useRef(null);
  React90.useEffect(() => {
    return () => {
      clearTimeout(startTimer.current);
    };
  }, []);
  const startCommit = React90.useCallback((params) => {
    const {
      pulsate: pulsate2,
      rippleX,
      rippleY,
  ...
  const start2 = React90.useCallback((event = {}, options = {}, cb = () => {
  }) => {
    const {
  ...
  const pulsate = React90.useCallback(() => {
    start2({}, {
      pulsate: true
    });
  }, [start2]);
  const stop = React90.useCallback((event, cb) => {
    clearTimeout(startTimer.current);
  ...
  React90.useImperativeHandle(ref, () => ({
    pulsate,
    start: start2,
    stop
  }), [pulsate, start2, stop]);
...
var ButtonBase = React91.forwardRef(function ButtonBase2(inProps, ref) {
  const props = useThemeProps2({
    props: inProps,
    name: "MuiButtonBase"
  });
  ...
  const rippleRef = React91.useRef(null);
  ...
  const keydownRef = React91.useRef(false);
...
var Button3 = React93.forwardRef(function Button4(inProps, ref) {
...
@mui/private-theming/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-LFMHNMJF.js.map
